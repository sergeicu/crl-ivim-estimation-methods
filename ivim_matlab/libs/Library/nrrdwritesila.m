function nrrdwritesila(filename, X, space, spacedirections,spaceorigin)
...
space = sprintf('space: %s',space);
spacedirections = sprintf('space directions: %s',spacedirections);
kinds=sprintf('kinds: domain domain domain');
% sizes and dimensions and encoding
datasizes = size(X);
datadim = size(datasizes,2);
spaceorigin= sprintf('space origin: %s',spaceorigin);
