function process_ivim_whole_slice_with_nii2(filepath, savedir, slice)

% add path to load_untouch_nii
addpath(genpath('/fileserver/fastscratch/ipek/software/NIfTI_20140122/'));
...
% read vtk images
bvals_temp = [0, 50, 100,200,400,600]';
NumOfImages=length(bvals_temp);
%bvalueImageFileNames=C{2};
bValsVec=bvals_temp;
%[~,sp,orig,~]=readVTK([bvalueImageFileNames{1}]); 
%a1=load_untouch_nii([bvalueImageFileNames{1}]); 

a1=load_untouch_nii(filepath); 
bImages = squeeze(a1.img); 
